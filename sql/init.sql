...
CREATE TABLE agencies (
    ...
    code VARCHAR(32) UNIQUE NOT NULL,  -- e.g., 'BLS', 'CENSUS', 'EUROSTAT'
    name TEXT NOT NULL,
    country VARCHAR(8),
    url TEXT
);
...
CREATE TABLE datasets (
    id SERIAL PRIMARY KEY,
    code VARCHAR(64) UNIQUE NOT NULL,  -- e.g., 'CPS', 'NHIS', 'EU-LFS'
    name TEXT NOT NULL,
    agency_id INTEGER REFERENCES agencies(id),
    description TEXT,
    frequency VARCHAR(32)  -- 'monthly', 'annual', etc.
);
...
CREATE TABLE methodology_changes (
    id SERIAL PRIMARY KEY,
    dataset_id INTEGER REFERENCES datasets(id) NOT NULL,
    change_type VARCHAR(64) NOT NULL,  -- 'questionnaire_redesign', 'classification_change', 'weighting_update', etc.
    effective_date DATE,
    description TEXT NOT NULL,
    impact_estimate TEXT,  -- e.g., "Increased reported rate by ~1.2 percentage points"
...
CREATE TABLE change_indicator_impacts (
    ...
    impact_direction VARCHAR(16),  -- 'increase', 'decrease', 'unknown'
...
CREATE TABLE documents (
    id SERIAL PRIMARY KEY,
    title TEXT NOT NULL,
    doc_type VARCHAR(64),  -- 'methodology_note', 'release_note', 'working_paper'
    agency_id INTEGER REFERENCES agencies(id),
    url TEXT,
