...
class OrchestratorAgent(Agent):
    ...
    @contextmanager
    def _temporary_env(self, overrides: dict[str, str] | None):
        if not overrides:
            yield
            return
    ...
    async def _emit_progress(
        ...
        try:
            ...
            self.log(f"Progress callback failed: {exc}", level=30)
            return
    ...
    async def process_claim(
        ...
        *,
        ...
        with self._temporary_env(runtime_overrides):
            # Step 1: Parse the claim
            await self._emit_progress(progress_callback, {"event": "parser_started"})
            self._log_message("User", "Auditor", "request", text)
            claim, parser_reasoning = await self.parser.parse_with_reasoning(text)
            if parser_reasoning:
                thinking_blocks.append({"agent": "parser", "text": parser_reasoning})
            await self._emit_progress(progress_callback, {"event": "parser_completed"})
            ...
            source_outputs_summary = [
                {
                    ...
                    "analysis": dict(output.analysis) if output.analysis else {},
                }
                for output in aggregated.source_outputs
            ]
            self.last_run_details = {
            ...
            await self._emit_progress(progress_callback, {"event": "editor_started"})
